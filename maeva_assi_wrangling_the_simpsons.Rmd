...
Your turn! The code below filtered for male characters and arranged the data in ascending order of `id`. Edit the code to filter for female characters and reorder their names in alphabetical order:
...
## a. select()
Now let's look at the `simpsons_episodes` data. It has the following variables:
...
Let's call our new variable `total_episode_count` and arrange the data by descending order.
...
```{r summarize2, eval=FALSE, exercise=TRUE, eval=FALSE, exercise.setup="cleaning-episodes"}
#Edit the code
...
The formula for the range would be max_imdb - min_imdb, which we plug in `mutate()`, and we'll call the resulting column `range_imdb_rating`:
...
```{r select2, eval=FALSE, exercise=TRUE, eval=FALSE, exercise.setup="cleaning-episodes"}
#Edit the code
...
#Joining script line data to character data
...
```{r join2, eval=FALSE, warning=FALSE, exercise=TRUE, exercise.setup="rename1"}
#Edit the code
...
```{r rename2, eval=FALSE, exercise=TRUE, exercise.setup="join2-solution"}
#Edit the code
...
```{r words3, exercise = TRUE, exercise.setup = "replace", eval = FALSE}
#Edit the code
simpsons_df |>
  summarize(total_word_count= sum(word_count),
            .by = c(...)) |>
...
Based on the line of best fit, there seems to be a positive weak linear relationship between the total number of words spoken in a season and the IMDb rating of the season.\
Remember that correlation does not mean causation! The season with the highest average IMDb rating had one of the lowest total word count. A lot of spoken words does not necessarily mean the jokes were good to warrant higher ratings.
...
*It's best to filter out characters with unreported gender here!*
